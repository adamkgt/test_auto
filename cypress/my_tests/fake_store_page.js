...
describe('User registration', () => {
  it('Incorrect registration of a new user', () => {
    ...
    cy.get('#customer_login > div.u-column2.col-2 > form > p:nth-child(5) > button').should('be.disabled'); 
    cy.url().should('include', '/moje-konto'); 
    cy.get('#password_strength').should('contains.text', 'Bardzo słabe - Proszę wpisać mocniejsze hasło.'); 
    })
   })
  ...
  describe('User login', () => {
    ...
    cy.fixture('dataLogin').then((dataLogin) => {
      ...
      cy.url().should('include', '/moje-konto'); 
      cy.get('.woocommerce-error').should('contains.text', 'Błąd: wpisano niepoprawne hasło dla użytkownika admin. Nie pamiętasz hasła?'); 
    })
  })
